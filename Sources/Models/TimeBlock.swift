...
final class TimeBlock: Taggable {
    ...
    var duration: TimeInterval { max(0, endDate.timeIntervalSince(startDate)) } // Cached duration in seconds
    ...
    func recalculateDuration() {
        // No-op since duration is a computed property now, but keeping to avoid breaking API
    }
}
